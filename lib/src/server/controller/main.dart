...
class MainController {
  ...
  static addMessage(App app, HttpRequest req) {
    ...
    return req.listen((List<int> buffer) {
      ...
      // Add the message.
      return Firebase
          .post('/messages_by_community/$community.json', JSON.encode(message),
              auth: authToken)
          .then((String name) {
        Firebase.patch('/communities/$community.json', {
          'updatedDate': now.toString()
        }, auth: authToken);
        Firebase.put('/messages/$name.json', fullData, auth: authToken).then(
            (_) {
        ...
        return crawler.getPreview(Uri.parse(uri)).then((Response res) {
          ...
          if (preview.imageOriginalUrl == null ||
            ...
            return Firebase
              ...
              if (itemMap['subject'] == null) updates['subject'] =
                  preview.title;
            ...
            return createTemporaryFile().then((File file) {
              ...
              return downloadFileTo(preview.imageOriginalUrl, file).then((_) {
                // Resize the image.
                return imageUtil.resize(file, width: 225, height: 125).then(
                    (File convertedFile) {
                  // Save the preview.
                  return Firebase
                      .post('/uri_previews.json', preview.toJson(),
                          auth: authToken)
                      .then((String name) {
                    Map updates = {};
                    updates['uriPreviewId'] = name;
                    if (itemMap['subject'] == null) updates['subject'] =
                        preview.title;
                    if (itemMap['body'] == null) updates['body'] =
                        preview.teaser;
                    ...
                    var extension = path
                        .extension(preview.imageOriginalUrl.toString())
                        .split("?")[0];
                    ...
                    return app.cloudStorageUtil
                        .uploadFile(convertedFile.path, gsBucket, gsPath,
                            public: true)
                        .then((_) {
                      return file.delete().then((_) {
                        // Update the preview with a reference to the cloud file.
                        preview.imageSmallLocation = gsPath;
                        Firebase.patch('/uri_previews/$name.json',
                            JSON.encode(preview.toJson()), auth: authToken);
                        // Return the preview information.
                        response.data = preview;
                        return response;
                      });
  ...
  static sendWelcomeEmail(App app, HttpRequest request) {
    var id =
        request.cookies.firstWhere((cookie) => cookie.name == 'session').value;
...
//  }
  ...
  static _sendNotifications(type, Map data, App app) {
    ...
    Future findCommentInfo() {
      if (isItem) return null;
      return Firebase.get('/items/$item/activities/comments/$id.json').then(
          (commentData) {
    ...
    if (isMessage) {
      return findMessage().then((_) => Future.wait([findMessageAuthorInfo(_)]))
